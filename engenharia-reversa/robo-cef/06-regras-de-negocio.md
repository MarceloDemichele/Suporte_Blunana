...
## Atualizacao 2026-07-07 - Executor Blunana PROD

Foi criado `crawler-interface/reverse-prod.ts` para orquestrar a engenharia reversa Blunana em producao. A mudanca nao altera regras de negocio do robo CEF; afeta o fluxo operacional auxiliar de coleta Blunana, com validacao de configuracao e geracao de evidencias em `outputs/{tipo}/blunana/prod/`.

## Atualizacao 2026-07-07 - Organizacao por dominio e ambiente

Os artefatos operacionais passaram a ser separados por tipo, dominio e ambiente via `config/paths.ts`: `outputs/{tipo}/{OUTPUT_DOMAIN}/{APP_ENV}`. O dominio padrao e `blunana`; artefatos dos robos devem usar `OUTPUT_DOMAIN=robos` e uma das pastas tipadas em `outputs/{tipo}/robos/{ambiente}`. A mudanca nao altera regras de negocio do robo CEF, mas altera a regra operacional de criacao e localizacao de evidencias, documentos temporarios e resultados de crawler.
