...
function exigir(nome) {
    ...
    if (!valor)
        throw new Error(`${nome} precisa estar configurado com o texto exibido na tela.`);
    return valor;
}
async function selecionar(page, index, value) {
...
async function opcoes(page, index) {
    const dialog = page.locator('[role="dialog"],.v-dialog').filter({ visible: true }).first();
    return dialog.locator("select").nth(index).locator("option").evaluateAll((items) => items.map((item) => (item.textContent || "").trim()).filter(Boolean));
}
async function opcoesVisuais(page, placeholder) {
    const dialog = page.locator('[role="dialog"],.v-dialog').filter({ visible: true }).first();
...
async function main() {
    ...
    const menu = JSON.parse(fs_1.default.readFileSync((0, paths_1.outputPath)("blunana-menu.json"), "utf-8"));
    const rota = (fim) => menu.find((item) => item.href && new URL(item.href).pathname.endsWith(`/${fim}`))?.href;
    const { browser, page } = await (0, login_1.loginBlunana)();
    try {
        const usuario = rota("configuracao_usuario");
        if (!usuario)
            throw new Error("Rota Configuracao Usuario nao encontrada.");
        ...
        for (const item of await opcoesVisuais(page, /selecione um papel/i))
            console.log(`- ${item}`);
        await page.keyboard.press("Escape");
        const prazo = rota("configuracao_de_prazo");
        if (!prazo)
            throw new Error("Rota Configuracao do Prazo nao encontrada.");
        ...
        for (const item of await opcoesVisuais(page, /selecione o tipo de prazo/i))
            console.log(`- ${item}`);
        await page.keyboard.press("Escape");
    }
    finally {
