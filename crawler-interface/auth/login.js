...
exports.loginBlunana = loginBlunana;
const session_1 = require("./session");
async function loginBlunana() {
    ...
    const exigir = (nome) => {
        ...
        if (!valor)
            throw new Error(`${nome} nao configurado em .env.${environment}.`);
        return valor;
    };
    ...
    const { browser, page } = await (0, session_1.criarSessao)();
    const network = [];
    ...
    try {
        ...
        if (page.url().includes("/auth/login") && await campoMFA.isVisible().catch(() => false)) {
            ...
            const confirmar = page.locator('button[type="submit"], button:has-text("Confirmar"), button:has-text("Validar")').first();
            await confirmar.waitFor({ state: "visible", timeout: 10000 });
            const aguardarRespostaOtp = () => page.waitForResponse((response) => response.request().method() === "POST" && response.url().includes("/Security/CompleteOtpLogin"), { timeout: 15000 });
