...
async function estrutura(page: Page, selector = "body") {
...
function lista(items: string[]) { return items.length ? items.map((x) => `- ${x}`).join("\n") : "- Nao identificado."; }
...
async function main() {
  ...
  try {
    for (const alvo of alvos) {
      ...
      const secoes: Array<{ aba: string; estrutura: Awaited<ReturnType<typeof estrutura>> }> = [];
      ...
      const modais: Array<{ origem: string; estrutura: Awaited<ReturnType<typeof estrutura>> }> = [];
      for (const secao of secoes) {
        ...
        if (!await abrir.isVisible().catch(() => false)) continue;
      ...
      fs.writeFileSync(outputPath(`${name}-detalhado.json`), JSON.stringify(result, null, 2), "utf-8");
      ...
      fs.writeFileSync(path.join(docsDir, `${name}.md`), md, "utf-8");
