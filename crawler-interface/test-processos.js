...
async function main() {
    ...
    if (!fs_1.default.existsSync(menuPath)) {
        throw new Error(`Menu ${environment_1.currentEnvironment.toUpperCase()} ausente. Execute npm run menu:${environment_1.currentEnvironment} primeiro.`);
    }
    ...
    if (!processos?.href)
        throw new Error("Rota PROD do menu Processos nao encontrada.");
    ...
    try {
        ...
        if (target.origin !== loginOrigin)
            throw new Error("Rota Processos fora da origem autorizada.");
        ...
        const sanitized = {
            ...
            environment: environment_1.currentEnvironment,
            mode: "read-only",
        ...
        fs_1.default.writeFileSync((0, paths_1.outputPath)("processos-tela.json"), JSON.stringify(sanitized, null, 2), "utf-8");
