...
async function main() {
  ...
  if (!processos?.href) throw new Error("Rota de Processos não encontrada no inventário PROD.");
  ...
  try {
    ...
    for (let indice = 0; indice < quantidade; indice += 1) {
      ...
      const codigo = await detalhe.evaluate(() => {
        ...
        if (correspondencia?.[1]) return correspondencia[1].trim();

        const elementos = Array.from(document.querySelectorAll("*"));
        const rotulo = elementos.find((elemento) =>
          elemento.children.length === 0 && elemento.textContent?.trim() === "Código do Cliente"
        );
        if (!rotulo) return "";
        let atual: Element | null = rotulo.parentElement;
        for (let nivel = 0; atual && nivel < 5; nivel += 1, atual = atual.parentElement) {
          ...
          if (posicao >= 0 && linhasTexto[posicao + 1]) return linhasTexto[posicao + 1];
        }
        return "";
      });
