...
function findOperationalActionGuide(question) {
    ...
    if (normativeIntent)
        return null;
    const hasOperationalAction = (guide) => guide.actionTerms.some((term) => text.includes(normalize(term))) ||
...
function answerOperationalAction(question, reference) {
    const guide = reference ? exports.operationalActionGuides.find((item) => item.id === reference) || null : findOperationalActionGuide(question);
    if (!guide)
        return null;
    if (guide.id !== "ACTION-CREATE-DEADLINE")
        return guide.answer;
    const text = normalize(question);
