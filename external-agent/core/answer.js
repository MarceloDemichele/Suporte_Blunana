...
const question_router_1 = require("./question-router");
function isDirectedEvidence(value) {
    return Boolean(value && typeof value === "object" && value.kind === "directed-query");
}
function formatBoolean(value) {
    return value ? "sim" : "não";
}
function formatDirectedAnswer(evidence) {
    if (!evidence.found) {
...
function possuiEvidencia(resultados, termos) {
...
function temIntencaoDeConsulta(pergunta) {
...
function gerarResposta(pergunta, resultados, runtimeEvidence = null, interpretation) {
    ...
    if (route.route === "LIVE_PLATFORM") {
        ...
        if (isDirectedEvidence(evidence))
            return formatDirectedAnswer(evidence);
