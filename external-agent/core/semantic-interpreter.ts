import { interpretQuestion, QuestionAction, QuestionEntity, QuestionIntent, QuestionInterpretation, RequestedField } from "./question-interpreter";
import { businessRules } from "./business-rules";
import { operationalActionGuides } from "./operational-actions";
import { findScreenConsultationGuide, screenConsultationGuides } from "./screen-consultation";

type FetchLike = typeof fetch;

const intents: QuestionIntent[] = ["CURRENT_DATA", "PROCEDURE", "SCREEN_LOOKUP", "BUSINESS_RULE", "UNKNOWN"];
const entities: QuestionEntity[] = ["USER", "PROCESS", "PUBLICATION", "DEADLINE", "HEARING", "BATCH_HEARING", "ATTESTATION", "DASHBOARD", "SCREEN", "UNKNOWN"];
const actions: QuestionAction[] = ["CREATE", "CONSULT", "CHECK_EXISTENCE", "CHECK_STATUS", "CHECK_PERMISSIONS", "EXPLAIN_RULE", "LOOKUP_FIELD", "UNKNOWN"];
const requestedFields: RequestedField[] = ["PROCESS_NUMBER", "CLIENT_CODE", "STATUS", "RESPONSIBLE", "PERMISSIONS", "VALUE", "ALL"];
const filterFields = ["PROCESS_NUMBER", "CLIENT_CODE", "USER_NAME", "UNKNOWN"] as const;
const allowedReferences = [
  ...operationalActionGuides.map((guide) => guide.id),
  ...screenConsultationGuides.map((guide) => guide.screen),
  ...businessRules.map((rule) => rule.id)
];
const procedureReferences = new Set(operationalActionGuides.map((guide) => guide.id));
const screenReferences = new Set(screenConsultationGuides.map((guide) => guide.screen));
const ruleReferences = new Set(businessRules.map((rule) => rule.id));
const capabilityCatalog = JSON.stringify({
  procedures: operationalActionGuides.map((guide) => ({ id: guide.id, action: guide.action })),
  screens: screenConsultationGuides.map((guide) => ({ reference: guide.screen, aliases: guide.aliases })),
  rules: businessRules.map((rule) => ({ id: rule.id, topic: rule.topic, example: rule.example }))
});
...
function normalizeCandidate(question: string, candidate: any, local: QuestionInterpretation): any {
  ...
  if (clientCode) {
    ...
    if (candidate.identifiers.processNumber === clientCode) candidate.identifiers.processNumber = null;
    candidate.filters = candidate.filters.filter((filter: any) => filter?.value !== clientCode);
    ...
    candidate.filters = candidate.filters.filter((filter: any) => filter?.value !== processNumber);
  ...
  const screenGuide = findScreenConsultationGuide(question);
  const screenSignal = [
  ...
  if (candidate.intent === "BUSINESS_RULE") candidate.action = "EXPLAIN_RULE";
  if (candidate.intent === "PROCEDURE") {
    candidate.action = "CREATE";
    if (candidate.reference === "ACTION-CREATE-DEADLINE" && candidate.sourceContext === "PUBLICATION") candidate.entity = "PUBLICATION";
    if (candidate.reference === "ACTION-CREATE-DEADLINE" && candidate.sourceContext === "HEARING") candidate.entity = "HEARING";
  }
  if (candidate.intent === "CURRENT_DATA") {
    ...
    if (existenceSignal && !(clientReference && asksProcessNumber)) {
      ...
      candidate.requestedFields = ["ALL"];
    } else if (candidate.requestedFields.includes("RESPONSIBLE") || candidate.requestedFields.includes("PROCESS_NUMBER") || candidate.requestedFields.includes("CLIENT_CODE")) candidate.action = "LOOKUP_FIELD";
    else if (candidate.requestedFields.includes("PERMISSIONS")) candidate.action = "CHECK_PERMISSIONS";
    else if (candidate.requestedFields.includes("STATUS")) candidate.action = "CHECK_STATUS";
  }
...
function validPlan(candidate: any): boolean {
  if (!candidate || !intents.includes(candidate.intent) || !entities.includes(candidate.entity) || !actions.includes(candidate.action)) return false;
  if (candidate.reference !== null && !allowedReferences.includes(candidate.reference)) return false;
  if (!Array.isArray(candidate.filters) || !Array.isArray(candidate.requestedFields)) return false;
  if (!candidate.filters.every((filter: any) => filterFields.includes(filter?.field) && filter?.operator === "EQUALS" && typeof filter?.value === "string" && filter.value.trim())) return false;
  if (!candidate.requestedFields.every((field: RequestedField) => requestedFields.includes(field))) return false;
  if (candidate.intent === "CURRENT_DATA" && candidate.filters.length === 0) return false;
  return typeof candidate.confidence === "number" && candidate.confidence >= 0 && candidate.confidence <= 1;
}

function supportedLivePlan(candidate: any): boolean {
  if (candidate.intent !== "CURRENT_DATA") return true;
  return candidate.filters.every((filter: any) => {
    if (candidate.entity === "USER") return filter.field === "USER_NAME";
    if (candidate.entity === "PROCESS" || candidate.entity === "PUBLICATION") return filter.field === "PROCESS_NUMBER" || filter.field === "CLIENT_CODE";
    return filter.field === "PROCESS_NUMBER";
  });
}

export async function interpretQuestionSemantic(question: string, fetchImpl: FetchLike = fetch): Promise<QuestionInterpretation> {
  const local = interpretQuestion(question);
  const apiKey = process.env.OPENAI_API_KEY?.trim();
  const mode = (process.env.QUESTION_INTERPRETER_MODE || "hybrid").toLowerCase();
  if (!apiKey || mode === "local") return local;
  ...
  try {
    ...
    if (!validPlan(candidate) || !supportedLivePlan(candidate)) return local;
