export type ScreenConsultationGuide = {
  screen: string;
  aliases: string[];
  answer: string;
  source: string;
...
export const screenConsultationGuides: ScreenConsultationGuide[] = [
  {
...
export function findScreenConsultationGuide(question: string): ScreenConsultationGuide | null {
  const text = normalize(question);
  const matches = screenConsultationGuides.flatMap((guide) =>
    guide.aliases.map((alias) => {
