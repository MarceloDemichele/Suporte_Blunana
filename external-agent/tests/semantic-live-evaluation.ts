...
import type { QuestionAction, QuestionEntity, QuestionIntent } from "../core/question-interpreter";
...
function evaluate(actual: Awaited<ReturnType<typeof interpretQuestionSemantic>>, expected: Expected): string[] {
  const failures: string[] = [];
  const compare = (field: string, value: unknown, wanted: unknown) => {
    if (wanted !== undefined && value !== wanted) failures.push(`${field}: esperado=${String(wanted)} obtido=${String(value)}`);
  };
  ...
  if (expected.filterField && !actual.filters.some((filter) => filter.field === expected.filterField)) failures.push(`filterField: ausente ${expected.filterField}`);
  if (expected.identifier) {
    const identifiers = Object.values(actual.identifiers).filter(Boolean);
    if (!identifiers.includes(expected.identifier)) failures.push(`identifier: ausente ${expected.identifier}`);
  }
  if (expected.requestedField && !actual.requestedFields.includes(expected.requestedField as never)) failures.push(`requestedField: ausente ${expected.requestedField}`);
  if (expected.intent !== "UNKNOWN" && actual.interpretationSource !== "MODEL") failures.push(`source: esperado=MODEL obtido=${actual.interpretationSource || "LOCAL"}`);
  return failures;
}

async function worker(queue: Array<{ item: EvaluationCase; index: number }>, results: any[]) {
  while (queue.length) {
    const next = queue.shift();
    if (!next) return;
    const started = Date.now();
    const actual = await interpretQuestionSemantic(next.item.question);
...
async function main() {
  ...
  for (const result of results) {
    ...
    if (!result.failures.length) categories[result.category].passed++;
  }
